...
//                }
    ...
    private void makeDescendingProductRequest(String cat_id) {
      ...
      ProgressDialog.show();
        ...
        CustomVolleyJsonRequest jsonObjReq = new CustomVolleyJsonRequest(Request.Method.POST,
            ...
            public void onResponse(JSONObject response) {
                ...
                try {
                    ...
                    if (status) {
                        ...
                        product_modelList = gson.fromJson(response.getString("data"), listType);
                        adapter_product = new Product_adapter(product_modelList, activity);
                        // rv_cat.setLayoutManager( new LinearLayoutManager(getActivity(),LinearLayoutManager.VERTICAL,false));
                        recyclerView.setAdapter(adapter_product);
                        adapter_product.notifyDataSetChanged();
                            if (product_modelList.isEmpty()) {
                                Toast.makeText(activity, getResources().getString(R.string.no_rcord_found), Toast.LENGTH_SHORT).show();
                            }
                            else
                            {
            ...
            public void onErrorResponse(VolleyError error) {
               ProgressDialog.dismiss();
               Module module=new Module(activity);
    ...
    private void makeAscendingProductRequest(String cat_id) {
        ...
        CustomVolleyJsonRequest jsonObjReq = new CustomVolleyJsonRequest(Request.Method.POST,
            ...
            @Override
            public void onResponse(JSONObject response) {
                Log.d("asscend", response.toString());
                ProgressDialog.dismiss();
                try {
                    Boolean status = response.getBoolean("responce");
                    if (status) {
